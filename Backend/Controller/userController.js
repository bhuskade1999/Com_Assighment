...
exports.deleteProfile = async (req, res) => {
  ...
  exports.upgradeUser = async (req,res)=>{
    ...
    if(user.is_premium_user){
      return res.status(200).send({success:true ,message:"Your Already an premium User"})
    }
    user.is_premium_user = true
    await user.save()
    return res.status(200).send({success:true ,message:"Congrats! Your are Upgraded to premium User"})
